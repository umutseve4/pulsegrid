...
from pulsegrid.model import ValidationResult, ValidationStatus
...
class M1InvariantTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.pipeline = Pipeline(self.store, clock=lambda: datetime(2026, 8, 28, 12, 30, tzinfo=UTC), id_factory=lambda: next(sequence))
    ...
    def counts(self) -> tuple[int, int, int, int]:
        return tuple(self.store.count(table) for table in ("bronze_attempts", "quarantine", "silver_events", "gold_events"))

    def test_schema_and_immutability_guards_exist(self) -> None:
        version = self.store.connection.execute("SELECT value FROM metadata WHERE key='schema_version'").fetchone()[0]
        objects = {(row["type"], row["name"]) for row in self.store.connection.execute("SELECT type, name FROM sqlite_master WHERE type IN ('table', 'trigger')")}
        self.assertEqual("1.0.0", version)
        expected = {"bronze_attempts": "immutable_bronze_event_id", "quarantine": "immutable_quarantine_event_id", "silver_events": "immutable_silver_event_id", "gold_events": "immutable_gold_event_id"}
    ...
    def test_unknown_version_fails_closed_without_silent_coercion(self) -> None:
        ...
        self.assertEqual("UNKNOWN_VERSION", self.store.quarantine_record(outcome.quarantine_id)["reason_code"])
        self.assertEqual((1, 1, 0, 0), self.counts())
    ...
    def test_duplicate_delivery_leaves_attempt_evidence_but_one_gold_row(self) -> None:
        ...
        duplicate_groups = self.store.connection.execute("SELECT event_id FROM gold_events GROUP BY event_id HAVING COUNT(*) > 1").fetchall()
    ...
    def test_replay_preserves_identity_and_uses_normal_validator(self) -> None:
        ...
        attempt = self.store.connection.execute("SELECT event_id, delivery_kind, parent_quarantine_id FROM bronze_attempts WHERE attempt_id = ?", (replayed.attempt_id,)).fetchone()
        ...
        self.assertEqual((invalid["event_id"], "replay", rejected.quarantine_id), tuple(attempt))
        self.assertIsNotNone(self.store.connection.execute("SELECT 1 FROM gold_events WHERE event_id = ?", (invalid["event_id"],)).fetchone())
    ...
    def test_transaction_rolls_back_bronze_when_trusted_write_fails(self) -> None:
        self.pipeline.validator = lambda _: ValidationResult(ValidationStatus.VALID, normalized_payload={"not_json_serializable": object()})
    ...
    def test_event_id_update_is_blocked_by_database_trigger(self) -> None:
        ...
        with self.assertRaises(sqlite3.IntegrityError):
            self.store.connection.execute("UPDATE gold_events SET event_id='mutated' WHERE event_id=?", (outcome.event_id,))
