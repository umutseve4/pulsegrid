...
class Pipeline:
    def __init__(self, store: EvidenceStore, *, validator: Callable[[Mapping[str, Any] | object], ValidationResult] = validate, clock: Callable[[], datetime] | None = None, id_factory: Callable[[], str] | None = None) -> None:
        self.store = store
    ...
    def ingest(self, envelope: Mapping[str, Any] | object, *, delivery_kind: str = "delivery", parent_quarantine_id: str | None = None) -> IngestOutcome:
        ...
        with self.store.transaction() as connection:
            connection.execute("INSERT INTO bronze_attempts VALUES (?, ?, ?, ?, ?, ?)", (attempt_id, event_id, envelope_json, delivery_kind, parent_quarantine_id, created_at))
            if not validation.accepted:
                quarantine_id = self.id_factory()
                connection.execute("INSERT INTO quarantine VALUES (?, ?, ?, ?, ?, ?, ?)", (quarantine_id, attempt_id, event_id, validation.status.value, json.dumps(validation.details), envelope_json, created_at))
            ...
            existing_gold = connection.execute("SELECT normalized_json FROM gold_events WHERE event_id = ?", (event_id,)).fetchone()
            if existing_gold is not None and existing_gold["normalized_json"] != normalized_json:
                quarantine_id = self.id_factory()
                connection.execute("INSERT INTO quarantine VALUES (?, ?, ?, ?, ?, ?, ?)", (quarantine_id, attempt_id, event_id, "IDENTITY_CONFLICT", json.dumps(("event_id already maps to different normalized content",)), envelope_json, created_at))
                return IngestOutcome(attempt_id, event_id, False, "IDENTITY_CONFLICT", quarantine_id)
            connection.execute("INSERT INTO silver_events VALUES (?, ?, ?, ?)", (attempt_id, event_id, normalized_json, created_at))
            cursor = connection.execute("INSERT INTO gold_events VALUES (?, ?, ?, ?) ON CONFLICT(event_id) DO NOTHING", (event_id, normalized_json, attempt_id, created_at))
            return IngestOutcome(attempt_id, event_id, True, gold_inserted=cursor.rowcount == 1)

    def replay(self, quarantine_id: str, corrected_envelope: Mapping[str, Any] | None = None) -> IngestOutcome:
        ...
        return self.ingest(candidate, delivery_kind="replay", parent_quarantine_id=quarantine_id)
